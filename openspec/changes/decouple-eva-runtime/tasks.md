...
- [x] 3.1 Refactor `BackendCoordinator` so it no longer stores `Widget *` and instead consumes runtime settings/state/event sinks.
...
- [x] 4.2 Move `ui_messagesArray`, history ownership, compaction state, KV counters, and active turn state into runtime/session state.
- [x] 4.3 Refactor `ToolFlowController` to emit runtime tool events instead of directly modifying Widget records.
...
- [x] 5.3 Remove duplicate Widget ownership of runtime truth after each migrated path is verified.
...
- [x] 6.2 Route ACP state, load, reset, models, and chat endpoints to runtime direct APIs.
