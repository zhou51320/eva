...
void applyCapabilityMetadata(QVector<ToolRegistry::Entry> &entries)
{
    ...
    applyOne(QStringLiteral("skill_run"), 1, 180000, true);
    applyOne(QStringLiteral("schedule_task"), 1, 30000, false);
    applyOne(QStringLiteral("stat_file"), 1, 30000, false);
    applyOne(QStringLiteral("copy_file"), 1, 60000, true);
    applyOne(QStringLiteral("artifact_confirm"), 1, 30000, false);
}
...
QVector<ToolRegistry::Entry> &mutableEntries()
{
    static QVector<ToolRegistry::Entry> entries = {
        ...
        {promptx::PROMPT_TOOL_EXECUTE,
         ...
         QStringLiteral(R"({"type":"object","properties":{"content":{"type":"string","description":"CLI command to execute. Alias: command."},"command":{"type":"string","description":"Alias for content."},"cwd":{"type":"string","description":"Working directory relative to the EVA workspace; defaults to workspace root."},"env":{"type":"object","additionalProperties":{"type":"string"},"description":"Environment variables to add or override for the command."},"timeout_ms":{"type":"integer","description":"Command timeout in milliseconds."},"shell":{"type":"string","description":"Shell hint: default, sh, bash, cmd, or powershell depending on platform."},"expected_outputs":{"type":"array","items":{"type":"string"},"description":"Files expected to be created, relative to the EVA workspace."},"label":{"type":"string","description":"Short purpose label for progress and diagnostics."}},"required":["content"]})"),
         QStringLiteral("Request to execute CLI commands on the system. Use only when a CLI or script is needed. Prefer cwd/env/timeout_ms/shell/expected_outputs/label instead of embedding brittle setup into the command. The observation is a Tool Result Envelope v2 JSON; inspect ok, data.stdout, data.stderr, data.exit_code, data.duration_ms, warnings, error, and recovery_hints before retrying. Identical repeated failures are flagged in data.repeated_failed_command."),
         QStringLiteral("请求在系统上执行 CLI 命令。仅在确实需要 CLI 或脚本时使用。优先使用 cwd、env、timeout_ms、shell、expected_outputs、label，而不是把脆弱的环境准备硬塞进命令。观察结果是 Tool Result Envelope v2 JSON；重试前检查 ok、data.stdout、data.stderr、data.exit_code、data.duration_ms、warnings、error、recovery_hints。重复相同失败会在 data.repeated_failed_command 标记。")},
        {promptx::PROMPT_TOOL_PTC,
         ...
         QStringLiteral("Programmatic Tool Calling lets you write a Python helper when CLI commands become unwieldy. EVA saves the file under ptc_temp, runs it immediately, and returns a Tool Result Envelope v2 JSON with stdout/stderr/exit details in data.legacy_text plus ok/error/recovery_hints for recovery decisions."),
         QStringLiteral("编程式工具调用：当 CLI 命令繁琐时，用 Python 编写助手脚本。EVA 会将文件保存到 ptc_temp 并立即运行，返回 Tool Result Envelope v2 JSON；stdout/stderr/exit 信息在 data.legacy_text 中，同时包含 ok/error/recovery_hints 供恢复判断。")},
        {promptx::PROMPT_TOOL_LIST_FILES,
         ...
         QStringLiteral("List all immediate subfolders and files under a directory. Prefer this structured path tool over shell ls/dir. The observation is a Tool Result Envelope v2 JSON; read data.legacy_text for the compact listing and error/recovery_hints for path failures."),
         QStringLiteral("列出目录下所有直接子文件夹和文件。常规路径检查优先使用此结构化工具而不是 shell ls/dir。观察结果是 Tool Result Envelope v2 JSON；紧凑列表在 data.legacy_text，路径失败查看 error/recovery_hints。")},
        {promptx::PROMPT_TOOL_SEARCH_CONTENT,
         ...
         QStringLiteral("Search text files under the engineer working directory for a query string. Prefer this structured search over shell grep/rg for routine checks. The observation is a Tool Result Envelope v2 JSON; matching lines are in data.legacy_text and failures include error/recovery_hints."),
         QStringLiteral("在工程师工作目录下搜索文本。常规检查优先使用此结构化搜索而不是 shell grep/rg。观察结果是 Tool Result Envelope v2 JSON；匹配行在 data.legacy_text，失败包含 error/recovery_hints。")},
        {promptx::PROMPT_TOOL_READ_FILE,
         ...
         QStringLiteral("Read file content from a workspace or approved Skill path. Prefer this structured file tool over shell cat/type. The observation is a Tool Result Envelope v2 JSON; file slices are in data.legacy_text and path/read failures include error/recovery_hints."),
         QStringLiteral("读取工作区或允许的 Skill 路径文件内容。常规读取优先使用此结构化文件工具而不是 shell cat/type。观察结果是 Tool Result Envelope v2 JSON；文件片段在 data.legacy_text，路径/读取失败包含 error/recovery_hints。")},
        {promptx::PROMPT_TOOL_WRITE_FILE,
         ...
         QStringLiteral("Write content to a file at the specified path, creating parent directories when needed. The observation is a Tool Result Envelope v2 JSON with ok/error/recovery_hints; verify generated artifacts separately when the user expects an output file."),
         QStringLiteral("将内容写入指定路径，必要时创建父目录。观察结果是 Tool Result Envelope v2 JSON，包含 ok/error/recovery_hints；用户期待输出产物时还需要单独确认 artifact。")},
        {promptx::PROMPT_TOOL_REPLACE_IN_FILE,
         ...
         QStringLiteral("Replace literal text inside an existing file. The observation is a Tool Result Envelope v2 JSON; success or failure details are in data.legacy_text, with error/recovery_hints when the old string, path, or expected count fails."),
         QStringLiteral("在现有文件中替换字面文本。观察结果是 Tool Result Envelope v2 JSON；成功或失败细节在 data.legacy_text，old_string、路径或期望次数失败时查看 error/recovery_hints。")},
        {promptx::PROMPT_TOOL_EDIT_IN_FILE,
         ...
         QStringLiteral("Apply structured line-based edits to an existing text file. The observation is a Tool Result Envelope v2 JSON; edit results are in data.legacy_text and failures include error/recovery_hints."),
         QStringLiteral("对现有文本文件应用结构化按行编辑。观察结果是 Tool Result Envelope v2 JSON；编辑结果在 data.legacy_text，失败包含 error/recovery_hints。")},
        ...
        {promptx::PROMPT_TOOL_SKILL_CALL,
         ...
         QStringLiteral("Return full SKILL.md and directory tree for the specified skill name. The observation is a Tool Result Envelope v2 JSON; parsed Skill data is in data.legacy_text, and missing Skill/path failures include error/recovery_hints."),
         QStringLiteral("根据技能名称返回完整 SKILL.md 及技能目录结构。观察结果是 Tool Result Envelope v2 JSON；Skill 数据在 data.legacy_text，缺失或路径失败包含 error/recovery_hints。")},
        {promptx::PROMPT_TOOL_SKILL_RUN,
         QStringLiteral("skill_run"),
         QStringLiteral(R"({"type":"object","properties":{"name":{"type":"string","description":"Skill name to run."},"action":{"type":"string","description":"Declared or inferred entrypoint action. Defaults to first entrypoint or run."},"args":{"type":"object","description":"Action arguments exposed as environment/diagnostic metadata."},"timeout_ms":{"type":"integer","description":"Execution timeout in milliseconds."},"expected_outputs":{"type":"array","items":{"type":"string"},"description":"Expected output files relative to the isolated run directory or workspace."}},"required":["name"]})"),
         QStringLiteral("Run a declared Skill command entrypoint in an isolated workspace run directory. Stages Skill assets read-only into the run directory and returns Tool Result Envelope v2 with status, logs, recovery hints, and artifacts."),
         QStringLiteral("在隔离的工作区运行目录中执行 Skill 声明的命令入口。会把只读 Skill 资产 staging 到运行目录，并返回包含状态、日志、恢复提示和产物的 Tool Result Envelope v2。")},
        ...
        {promptx::PROMPT_TOOL_STAT_FILE,
         QStringLiteral("stat_file"),
         QStringLiteral(R"({"type":"object","properties":{"path":{"type":"string","description":"File or directory path relative to the engineer workspace, or an approved Skill path."}},"required":["path"]})"),
         QStringLiteral("Stat a file or directory with normalized path, type, size, modified time, and allowed-root metadata. Prefer this structured path tool before retrying path-sensitive commands."),
         QStringLiteral("查看文件或目录 metadata，返回规范化路径、类型、大小、修改时间和允许根信息。路径相关命令重试前优先使用此结构化路径工具。")},
        {promptx::PROMPT_TOOL_COPY_FILE,
         QStringLiteral("copy_file"),
         QStringLiteral(R"({"type":"object","properties":{"source":{"type":"string","description":"Source file or directory path inside the workspace or approved Skill roots."},"destination":{"type":"string","description":"Destination path inside the engineer workspace."},"overwrite":{"type":"boolean","description":"Overwrite an existing destination when true."}},"required":["source","destination"]})"),
         QStringLiteral("Copy a file or directory safely into the workspace. Handles spaces and non-ASCII paths, normalizes metadata, and rejects path or symlink escapes."),
         QStringLiteral("将文件或目录安全复制到工作区。支持空格和中文路径，返回规范化 metadata，并拒绝越权路径或符号链接逃逸。")},
        {promptx::PROMPT_TOOL_ARTIFACT_CONFIRM,
         QStringLiteral("artifact_confirm"),
         QStringLiteral(R"({"type":"object","properties":{"path":{"type":"string","description":"Generated artifact path to confirm."},"label":{"type":"string","description":"Display label for the artifact."},"source_tool":{"type":"string","description":"Tool or step that produced the artifact."}},"required":["path"]})"),
         QStringLiteral("Confirm a generated artifact before claiming completion. Returns path, normalized path, size, type/extension, modified time, source tool, label, and open/download hints."),
         QStringLiteral("在声称生成任务完成前确认产物。返回路径、规范化路径、大小、类型/扩展名、修改时间、来源工具、标签和打开/下载提示。")},
    };
